  ...
  AuthBloc() : super(AuthStateUninitialize()) {
    ...
    on<AuthEventPerformLogin>((event, emit) async {
      ...
      try {
        ...
        if (e is DataExceptionAvatarRequired) {
          emit(AuthStateAvatarRequired());
          return;
        }
    ...
    on<AuthEventAppleLogin>((event, emit) async {
      try {
        ...
        debugPrint("[debug AppleLogin] ${e.message}");
        emit(AuthStateLoginFailure(exception: e));
      }
    });
    ...
    on<AuthEventGoogleLogin>((event, emit) async {
      ...
      try {
        ...
        debugPrint("[debug GoogleLogin] ${e.message}");
        emit(AuthStateLoginFailure(exception: e));
      }
    });

    /// On Email Verification Event
    on<AuthEventValidateEmailVerification>(
      (event, emit) {
        emit(AuthStateValidateEmailVerification(
            isEmailVerified: AuthRepo().didEmailVerified()));
      },
    );
  }
}
